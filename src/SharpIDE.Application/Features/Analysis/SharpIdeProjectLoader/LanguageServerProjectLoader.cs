...
using SharpIDE.Application.Features.Analysis.SharpIdeProjectLoader;
...
internal abstract class LanguageServerProjectLoader
{
    ...
    protected readonly ImmutableDictionary<string, string> AdditionalProperties = ImmutableDictionary<string, string>.Empty;
    ...
    private async ValueTask ReloadProjectsAsync(ImmutableSegmentedList<ProjectToLoad> projectsToLoadOrReload, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var enableAutomaticRestore = false;
            if (enableAutomaticRestore && projectsThatNeedRestore.Any())
            {
                // This request blocks to ensure we aren't trying to run a design time build at the same time as a restore.
                await ProjectDependencyHelper.RestoreProjectsAsync(projectsThatNeedRestore, EnableProgressReporting, _logger, cancellationToken);
            }
        }
        finally
        {
