...
internal static class ProjectDependencyHelper
{
    ...
    private static bool CheckProjectAssetsForUnresolvedDependencies(ProjectFileInfo projectFileInfo, ILogger logger)
    {
        ...
        if (!File.Exists(projectAssetsPath))
        {
            ...
            logger.LogWarning("Unresolved dependencies in {ProjectFilePath}", projectFileInfo.FilePath);
            return true;
        }
        ...
        if (unresolved.Any())
        {
            var message = $"Unresolved dependencies in {projectFileInfo.FilePath}"
                          + Environment.NewLine
    ...
    internal static async Task RestoreProjectsAsync(ImmutableArray<string> projectPaths, bool enableProgressReporting, ILogger logger, CancellationToken cancellationToken)
    {
